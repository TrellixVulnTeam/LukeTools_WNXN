...
class CreateToolsetsPanel(nukescripts.PythonPanel): 
  ...
  def createPreset(self): 
    if nuke.createToolset(str(self.menuPath.value()), rootPath = self.rootDir): 
...
def refreshToolsetsMenu():   
  ...
  m = toolbar.findItem("L_ToolSets") 
  if m != None: 
    m.clearMenu() 
    createToolsetsMenu(toolbar) 
 
def createToolsetsMenu(toolbar): 
  m = toolbar.addMenu("L_ToolSets", "ToolbarToolsets.png") 
  m.addCommand("Create", "L_ToolSets.addToolsetsPanel()", "", icon="ToolsetCreate.png") 
  m.addCommand("-", "", "") 
...
def traversePluginPaths(m, delete, allToolsetsList, isLocal): 
  ...
  for d in nuke.pluginPath(): 
    ...
    if ignore: 
      continue 
    if (not isLocal) and (d.find("LukeTools") != -1): 
      continue 
    ...
    for group in filecontents: 
      ...
      if not os.path.isdir(fullFileName): 
        ...
        if extPos != -1 and extPos == len(group) - 3: 
          ...
          if delete: 
            m.addCommand(group, 'L_ToolSets.deleteToolset("%s", "%s")' % (rootPath, fullFileName), "") 
            retval = True 
          else: 
