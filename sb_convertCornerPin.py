...
def getCornerPinMatrixAtFrame(node, frame, refFrame):
    ...
    projectionMatrixTo.mapUnitSquareToQuad(
        toValues[0], toValues[1], toValues[2], toValues[3], toValues[4], toValues[5], toValues[6], toValues[7])
    projectionMatrixFrom.mapUnitSquareToQuad(
        fromValues[0], fromValues[1], fromValues[2], fromValues[3], fromValues[4], fromValues[5], fromValues[6], fromValues[7])
...
def createTrackerHack(numTracks, startFrame):
    ...
    addTrack = """{ {curve K x%s 1} "track 1" {curve x%s 630} {curve x%s 200} {curve K x%s 0} {curve K x%s 0} 1 0 0 {curve x%s 0} 0 0 -32 -32 32 32 -22 -22 22 22 {} {curve x%s 576} {curve x%s 146} {curve x%s 683} {curve x%s 253} {curve x%s 598} {curve x%s 168} {curve x%s 661} {curve x%s 231} {curve x%s 31.5} {curve x%s 31.5}  }""" % (
        startFrame, startFrame, startFrame, startFrame, startFrame, startFrame, startFrame, startFrame, startFrame, startFrame, startFrame, startFrame, startFrame, startFrame, startFrame, startFrame)
...
class sb_convertCornerPin_Panel(nukescripts.PythonPanel):
    ...
    def __init__(self):
        ...
        nukescripts.PythonPanel.__init__(self, '{0} v{1}'.format(
            scriptData["scriptName"], scriptData["scriptVersion"]))
        ...
        self.output = nuke.Enumeration_Knob("output", "output", [
                                            "RotoPaint", "SplineWarp", "GridWarp", "CornerPin Match-move (Plane)", "CornerPin Stabilize (Plane)", "Tracker"])
